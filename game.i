...
void updatePlayer() {
 if ((~((*(volatile unsigned short *)0x04000130)) & ((1<<5)))
  ...
  && 240 - 10 - player.cdel > player.col) {
  player.col += player.cdel;
        direction = 2;

 } if ((~((*(volatile unsigned short *)0x04000130)) & ((1<<6))) && player.row - player.size >= player.rdel) {
        player.row -= player.rdel;
        direction = 3;

    } if ((~((*(volatile unsigned short *)0x04000130)) & ((1<<7))) && player.row + player.size < 160 - player.rdel) {
        player.row += player.rdel;
        direction = 4;
    }
 ...
 if ((!(~(oldButtons)&((1<<1))) && (~buttons & ((1<<1)))) && player.bulletTimer >= 20) {
   fireBullet();
...
void drawPlayer() {
    if (prevdirection == 1) {
        drawArrowLeft(player.oldCol, player.oldRow, 10, 0);
    } else if (prevdirection == 2) {
        drawArrowRight(player.oldCol, player.oldRow, 10, 0);
    } else if (prevdirection == 3 || prevdirection == 0) {
        drawArrowUp(player.oldCol, player.oldRow, 10, 0);
    } else if (prevdirection == 4) {
        drawArrowDown(player.oldCol, player.oldRow, 10, 0);
    }
...
void updateBullet(BULLET* b) {
    if (b->active == 1) {
        if (b->col > 0 && b->direction == 1) {
            ...
            b->active = 0;
        } else if (b->col < 240 && b->direction == 2) {
            b->col = b->col + 1;
        } else if (b->direction == 2){
            b->active = 0;
        } else if (b->row < 160 && b->direction == 4) {
            b->row = b->row + 1;
